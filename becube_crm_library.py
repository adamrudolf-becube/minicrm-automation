...
class CrmData:
    ...
    def set_course_states(self):
        ...
        for course in course_list:
            ...
            try:
                if self.today >= datetime.datetime.strptime(course_data["ElsoAlkalom"], "%Y-%m-%d %H:%M:%S"):
                    trace("Set: ElsoAlkalom")
                    update_data["StatusId"] = self.tanfolymok.get_status_number_by_name("Folyamatban")
                if self.today >= datetime.datetime.strptime(course_data["N10Alkalom"], "%Y-%m-%d %H:%M:%S"):
                    trace("Set: N10Alkalom")
                    update_data["StatusId"] = self.tanfolymok.get_status_number_by_name("Frissen végzett")
                if self.today >= datetime.datetime.strptime(course_data["N10Alkalom"], "%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=35):
    ...
    def send_scheduled_emails(self):
        ...
        for student in student_list:
            ...
            if self.today >= datetime.datetime.strptime(student_data["N10Alkalom2"], "%Y-%m-%d %H:%M:%S"):
            ...
            if student_data["N2SzunetOpcionalis2"] != "":
                if self.today >= datetime.datetime.strptime(student_data["N2SzunetOpcionalis2"], "%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=-2):
            ...
            if student_data["N2SzunetOpcionalis3"] != "":
                if self.today >= datetime.datetime.strptime(student_data["N2SzunetOpcionalis3"], "%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=-2):
            ...
            if student_data["N3SzunetOpcionalis2"] != "":
                if self.today >= datetime.datetime.strptime(student_data["N3SzunetOpcionalis2"], "%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=-2):
