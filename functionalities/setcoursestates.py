...
def set_course_states(crm_data):
    ...
    """
    open_course_list = crm_data.get_course_list_with_status(APPLICATION_OPEN_STATE)
    ongoing_course_list = crm_data.get_course_list_with_status(IN_PROGRESS_STATE)
    freshly_finished = crm_data.get_course_list_with_status(RECENTLY_FINISHED_STATE)
    ...
    for course in course_list:
        ...
        try:
            if crm_data.get_today() >= datetime.datetime.strptime(course_data[FIRST_OCCASION_FIELD_NAME], DATE_FORMAT):
                trace("Set: " + FIRST_OCCASION_FIELD_NAME)
                update_data[STATUS_ID_FIELD] = crm_data.get_course_status_number_by_name(IN_PROGRESS_STATE)
            if crm_data.get_today() >= datetime.datetime.strptime(course_data[LAST_OCCASION_FIELD_NAME], DATE_FORMAT):
                trace("Set: " + LAST_OCCASION_FIELD_NAME)
                update_data[STATUS_ID_FIELD] = crm_data.get_course_status_number_by_name(RECENTLY_FINISHED_STATE)
            if crm_data.get_today() >= datetime.datetime.strptime(course_data[LAST_OCCASION_FIELD_NAME],
                                                                  DATE_FORMAT) + datetime.timedelta(days=35):
                trace("Set: " + LAST_OCCASION_FIELD_NAME + " + 35 nap")
                update_data[STATUS_ID_FIELD] = crm_data.get_course_status_number_by_name(FINISHED_STATE)
        except:
            trace("Missing date")
