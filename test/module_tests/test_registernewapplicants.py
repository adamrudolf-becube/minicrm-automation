...
class TestRegisterNewApplicants(MiniCrmTestBase):
    ...
    def test_frontend_student_is_applied_headcount_is_less_than_the_limit_put_student_to_infosent_update_headcounts_copy_course_data(
            self):
        """
        test_frontend_student_is_applied_headcount_is_less_than_the_limit_put_student_to_infosent_update_headcounts_copy_course_data
        ...
        self.request_handler.expect_request(
            crmrequestfactory.get_student(FAKE_STUDENT_ID_NUMBER),
            responses_students.FAKE_STUDENT_FRONTEND_ZERO_MAILS)
        ...
        self.request_handler.expect_request(
            crmrequestfactory.get_course(FAKE_COURSE_ID_NUMBER),
            responses_courses.COURSE_2019_1_Q_FRONTEND_ONE_PLACE_FREE
        )
        self.request_handler.expect_request(
            crmrequestfactory.set_project_data(
                FAKE_STUDENT_OTHER_ID_NUMBER,
        ...
        self.request_handler.expect_request(
            crmrequestfactory.set_project_data(
                FAKE_STUDENT_OTHER_ID_NUMBER,
                {u"StatusId": u"2781", u"Levelkuldesek": u"Frontend INFO lev\u00e9l"}
            ),
            responses_general.XPUT_RESPONSE
        )
        register_new_applicants(self.crm_facade)

    def test_online_frontend_student_is_applied_headcount_is_less_than_the_limit_put_student_to_infosent_update_headcounts_copy_course_data(
            self):
        """
        test_online_frontend_student_is_applied_headcount_is_less_than_the_limit_put_student_to_infosent_update_headcounts_copy_course_data

        Given:
            - one online frontend student is in applied ("Jelentkezett") state
            - current headcount is less than maximal headcount. (there is at least one free spot) in the wanted course.)
        When:
            - register_new_applicants() is called
        Then:
            - student's info is filled
            - student is put to INFO sent ("INFO level kiment") state
            - online frontend INFO mail is sent
            - application deadline is set
            - headcount is updated
        """
        ...
        self.request_handler.expect_request(
            crmrequestfactory.get_course(FAKE_COURSE_ID_NUMBER),
            responses_courses.COURSE_2019_1_Q_ONLINE_FRONTEND_ONE_PLACE_FREE
        )
        self.request_handler.expect_request(
            crmrequestfactory.set_project_data(
                FAKE_STUDENT_OTHER_ID_NUMBER,
        ...
        self.request_handler.expect_request(
            crmrequestfactory.set_project_data(
                FAKE_STUDENT_OTHER_ID_NUMBER,
                {u"StatusId": u"2781", u"Levelkuldesek": u"Online frontend INFO lev\u00e9l"}
            ),
            responses_general.XPUT_RESPONSE
        )
        register_new_applicants(self.crm_facade)
    ...
    def test_beginner_student_is_applied_headcount_is_not_less_than_the_limit_put_student_to_waiting_list_and_send_mail(
            self):
        """
    ...
    def test_advanced_student_is_applied_headcount_is_not_less_than_the_limit_put_student_to_waiting_list_and_send_mail(
            self):
        """
    ...
    def test_frontend_student_is_applied_headcount_is_not_less_than_the_limit_put_student_to_waiting_list_and_send_mail(
            self):
        """
        ...
        self.request_handler.expect_request(
            crmrequestfactory.get_course(FAKE_COURSE_ID_NUMBER),
            responses_courses.COURSE_2019_1_Q_FRONTEND_FULL)
        self.request_handler.expect_request(
            crmrequestfactory.set_project_data(
                FAKE_STUDENT_OTHER_ID_NUMBER,
                {u"TanfolyamKodja": u"2019-1-Q"}
