...
class TestCommandHandlerMock(unittest.TestCase):
    def setUp(self):
        self.command_handler = CommandHandlerMock("FakeApi", "FakeApi")
    ...
    def test_match_expectation_raises_if_there_is_no_expectation(self):
        with self.assertRaisesRegexp(AssertionError, "No more commands were expected, but got") as cm:
            self.command_handler.match_expectation(self.crm_command_factory.get_student(42))
    ...
    def test_match_expectation_raises_when_other_command_comes(self):
        ...
        with self.assertRaisesRegexp(AssertionError, "Unexpected command") as cm:
            self.command_handler.match_expectation(self.crm_command_factory.get_student(137))
    ...
    def test_expectations_are_matched_in_correct_order(self):
        self.command_handler.expect_command(self.crm_command_factory.get_student(42), 'b')
        self.command_handler.expect_command(self.crm_command_factory.get_student(43), 'd')
        self.command_handler.expect_command(self.crm_command_factory.get_student(44), 'f')
        self.command_handler.match_expectation(self.crm_command_factory.get_student(42))
        self.command_handler.match_expectation(self.crm_command_factory.get_student(43))
        self.command_handler.match_expectation(self.crm_command_factory.get_student(44))
        self.command_handler.check_is_satisfied()
